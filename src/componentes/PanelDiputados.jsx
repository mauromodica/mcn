...
export default function PanelDiputados({ diputados, ubicaciones, abierto, onDropAlPanel }) {
  const [busqueda, setBusqueda] = useState('');

  const texto = normalizar(busqueda);

  const diputadosUbicados = Object.keys(ubicaciones ?? {});
  const ausentes = diputados.filter((d) => !diputadosUbicados.includes(d.id));

  const diputadosFiltrados = ausentes.filter((dip) => {
    // Construyo un mapa de claves normalizadas -> valor original (trim)
    const camposNorm = {};
    ...
    // Intento obtener nombre y apellido a partir de varias claves posibles
    const nombreRaw =
      camposNorm['nombre'] ||
      camposNorm['nombres'] ||
      ...
      camposNorm['givenname'] || // por si viene con otro encabezado
      '';
    ...
    // Normalizo los valores para comparar (sin tildes, minúsculas)
  ...
  const mostrarDiputados = texto === '' ? ausentes : diputadosFiltrados;
  ...
  const [{ isOver, canDrop }, drop] = useDrop(
    () => ({
      ...
      canDrop: (item) => diputadosUbicados.includes(item.id),
      collect: (monitor) => ({
        isOver: monitor.isOver(),
        canDrop: monitor.canDrop(),
      }),
    }),
    [diputadosUbicados, onDropAlPanel]
  );
